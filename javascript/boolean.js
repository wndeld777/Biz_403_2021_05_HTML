...
/* 
    Java와 같은 경우는 if() 문에서
    Single EQ(=)를 사용하면 syntax error 가 난다
    if(num=3){}
...
// 논리적으로 false가 인식되고 그 외에는 무조건 true이다
if (0) {
  console.log(true);
} else {
  console.log(false);
...
동등연산자( == )
    자동으로 형변환이 발생한다
    0 == '0'을 비교를 하면 문자열 '0'을 숫자 0으로 자동형변환을 해 버린다
...
// 코드가 다소 복잡해 지지만 강제 형변환을 하고 일치연산자로 비교하는 것이
// 논리적 오류를 막을수 있다
