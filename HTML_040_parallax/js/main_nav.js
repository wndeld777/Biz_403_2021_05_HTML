document.addEventListener("DOMContentLoaded", () => {
  ...
  이벤트가 시작되어 화면에 그리는 코드가 실행될때 flag를 세팅하여 다시 코드가 중복 실행되는 것을 방지
  */
  let ticking = false;
  const scrollTop_nav_tick = (e) => {
    if (!ticking) {
      ...
      // 현재 선택된 tag(art에 담겨 있는 tag)에서 id 값을 추출하여(art1, art2)
